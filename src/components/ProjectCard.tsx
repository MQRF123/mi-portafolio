  ...
  return (
    <article className="glass-strong group relative flex flex-col overflow-hidden rounded-2xl p-7 transition-all duration-500 hover:-translate-y-1 hover:border-[var(--accent)] hover:shadow-[0_24px_80px_-20px_var(--accent-glow),0_0_0_1px_var(--accent)]">
      ...
      <div
        ...
        style={{ background: "radial-gradient(700px circle at 50% 0%, rgba(255,45,58,0.18), transparent 45%)" }}
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -inset-px rounded-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100"
        style={{ boxShadow: "0 0 24px rgba(255,45,58,0.35), inset 0 0 24px rgba(255,45,58,0.08)" }}
      />

      <div className="relative flex items-center justify-between">
        <span className="font-mono text-[10px] font-medium tracking-[0.25em] text-[var(--muted)]">
          {t("prefix")} / {project.code}
        </span>
        <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[var(--border-strong)] text-[var(--muted)] transition-all group-hover:border-[var(--accent)] group-hover:text-[var(--accent)] group-hover:shadow-[0_0_16px_var(--accent-glow)]">
          <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div className="relative mt-5 flex items-center justify-between border-t border-[var(--border)] pt-5">
        ...
        >
          <span aria-hidden className="h-[2px] w-5 m-stripe rounded-full opacity-60 transition-all duration-300 group-hover/btn:w-8 group-hover/btn:opacity-100" />
          {t("viewDetails")}
          <svg viewBox="0 0 16 16" className="h-3 w-3 transition-transform duration-300 group-hover/btn:translate-x-1" fill="none" stroke="currentColor" strokeWidth="2">
